...
export function ChannelMutateDrawer({
  ...
  // Platform-configured channel names, keyed by channel type. Suppliers must
  // pick from these; admins get them as suggestions.
  ...
  useEffect(() => {
    if (!isSupplier || !channelNamesByType) return
  ...
  }, [channelNamesByType, currentType, form, isSupplier])
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  {isSupplier || channelNameOptions.length > 0 ? (
                    <Combobox
                      options={channelNameOptions}
                      value={field.value}
                      onValueChange={(value) => field.onChange(value ?? '')}
                      placeholder={t('Select channel name')}
                      searchPlaceholder={t('Search channel name...')}
                      emptyText={t(
